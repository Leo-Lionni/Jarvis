    ...
    : QSyntaxHighlighter(parent)
{
    ...
    cppFormat.setForeground(Qt::darkGreen);
...
//                       sizeof                       union                       case                       enum
//                       mutable                       static                       unsigned                       catch
//                       explicit                       namespace                       static_cast                       using
//                       char                       export                       new                       struct
//                       virtual                       class                       extern                       operator
//                       switch                       void                       const                       false
//                       private                       template                       volatile
//                       const_cast                       float                       protected                       this
//                       wchar_t                       continue                       for                       public
//                       throw                       while                       default                       friend
//                       register                       true                       delete                       goto
                    <<"\\breinterpret_cast\\b"<<"\\btry\\b"<<"\\breturn\\b";
    foreach (const QString &pattern, keywordPatterns) {
        rule.pattern = QRegExp(pattern);
        rule.format = keywordFormat;
        highlightingRules.append(rule);
    ...
    rule.pattern = QRegExp("[\"<].*[\">]");//所有的"头文件"
    rule.format = quotationFormat;
    highlightingRules.append(rule);

    functionFormat.setFontItalic(true);
    functionFormat.setForeground(Qt::blue);
    rule.pattern = QRegExp("\\b[A-Za-z0-9_]+(?=\\()");//所有（前的单词,函数
